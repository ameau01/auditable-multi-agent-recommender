...
The audit trail spans two append-only tables in one SQLite file: `audit_records` for the agent's reasoning and `harness_trail` for what the harnesses verified or rejected. The split keeps each table focused on one audience; full schema lives in [`audit-trail.md`](audit-trail.md).
...
**Symmetric `related_event_id` linkage.** Every harness verdict that judges a specific audit row carries that row's id in `harness_trail.related_event_id`, populated via `AuditStore.link_harness_to_event`. The same UPDATE pattern handles all three timing shapes: the Action Harness fires *before* the `tool_call` row exists (so the dispatcher writes the row and then backfills the link), the Reasoning Harness fires before the supervisor decision or system-mapper output exists (same pattern, in the agent node), and the Orchestration Harness fires before `cycle_completed` exists (same pattern, in the runner). Uniform linkage means a reader of `harness_trail` can answer "which audit row did this verdict check?" for any harness type with one column read — no parsing of `details`, no joins on event-type strings. Two narrow exemptions: `cycle_started` rows have no antecedent to link to, and an Input-Harness rejection still records the rule it tripped without an audit row (the rejection is, by design, the absence of one).
