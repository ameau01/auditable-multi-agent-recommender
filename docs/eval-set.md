...
- **API key required.** The judge calls Anthropic. Set `ANTHROPIC_API_KEY` in the environment, typically loaded from a `.env` file. When the key is not set, Mid and Rich return `(skipped)` and the deterministic layers (Shape + Correctness) still run normally. This degradation keeps the report format identical whether or not the judge runs.
- **Provider and model configurable.** `LLM_JUDGE_PROVIDER` must be `anthropic` (the only currently supported provider — see [`decisions.md`](decisions.md)). `LLM_JUDGE_MODEL` overrides the default model; the default is `claude-haiku-4-5-20251001`. The default thresholds (Mid >= 30, Rich >= 60) were calibrated against this model; re-run `tests/judge_live/` after any model change to confirm gold-vs-gold scores still clear the high-richness band.
